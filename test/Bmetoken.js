...
contract('BmeToken', function(accounts){
    ...
    it('initiatizes the contract with the correct values', function(){
    	return BmeToken.deployed().then(function(instance){
    		tokenInstance = instance;
    		return tokenInstance.name();
    	}).then(function(name){
    		assert.equal(name, 'BME Token', 'has the correct name');
    		return tokenInstance.symbol();
    	}).then(function(symbol){
    		assert.equal(symbol, 'BMEt', 'has the correct symbol');
    		return tokenInstance.standard();
    	}).then(function(standard){
    		assert.equal(standard, 'BME Token v1.0', 'has the correct standard');
    	});
    })
	...
	it('transfers token ownership', function(){
		return BmeToken.deployed().then(function(instance){
            tokenInstance = instance;
            return tokenInstance.transfer.call(accounts[1], 9999999999999999);	// not transaction just try function work	
		}).then(assert.fail).catch(function(error){
			assert(error.message, 'error message must contain revert');
			return tokenInstance.transfer.call(accounts[1], 250000, {from: accounts[0]});
		}).then(function(success){
			assert.equal(success, true, 'its return true');
			return tokenInstance.transfer(accounts[1], 250000, {from: accounts[0]}); // actual transaction
			...
			assert.equal(balance.toNumber(), 750000, "deducts the ammount from the sending ammount");
		});
	});
	...
	it('approves tokens for delegated transfer', function(){
        return BmeToken.deployed().then(function(instance){
			...
			assert.equal(receipt.logs[0].args._spender, accounts[1], 'logs the account the tokens are auhorized to');
	...
	it('handles delegated token transfer', function(){
		return BmeToken.deployed().then(function(instance){
			...
			//approve spendAccount to spend 10 tokens from fromAccount
			return tokenInstance.approve(spendAccount, 10, {from: fromAccount});
		}).then(function(receipt){
			//try transferring sth larger than the sender's balance
			return tokenInstance.transferfrom(fromAccount, toAccount, 99999, {from: spendAccount});
		}).then(assert.fail).catch(function(error){
			...
			assert.equal(receipt.logs[0].args._to, toAccount, 'logs the account the tokens are auhorized to');
